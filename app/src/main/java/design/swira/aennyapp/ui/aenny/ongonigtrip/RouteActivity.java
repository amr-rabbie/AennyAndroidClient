...
public class RouteActivity extends AppCompatActivity implements View.OnClickListener , OnLocationFavouriteClick , OnRecentListClick {
    ...
    public void bindRecenetData(){
        ...
        onGoingViewModel.completedtripslistMutableLiveData.observe(RouteActivity.this, new Observer<List<CompletedTripResponse>>() {
            @Override
            public void onChanged(List<CompletedTripResponse> completedTripResponses) {
                if(completedTripResponses != null){
                    if(completedTripResponses.size() > 0) {
                        ...
                        binding.recentrecycler.setLayoutManager(new LinearLayoutManager(RouteActivity.this));
    ...
    private void getcuraddress2() {
        Geocoder geocoder;
        List<Address> addresses;
        geocoder = new Geocoder(this, Locale.getDefault());

        try {
            addresses = geocoder.getFromLocation(late, longg, 1); // Here 1 represent max location result to returned, by documents it recommended 1 to 5
            String address = addresses.get(0).getAddressLine(0); // If any additional address line present than only, check with max available address lines by getMaxAddressLineIndex()
            String city = addresses.get(0).getLocality();
            String state = addresses.get(0).getAdminArea();
            String country = addresses.get(0).getCountryName();
        ...
        switch (requestCode) {
            ...
            case 1:
                ...
                if (data != null) {
                    ...
                    longg=(double) data.getExtras().get("long");
                    late=(double) data.getExtras().get("late");
                    getcuraddress2();
